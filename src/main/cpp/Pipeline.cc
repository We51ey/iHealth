...
//  agcn_.reset(new AGCN(modelDir, labelPath, cpuThreadNum, cpuPowerMode, 0.2));
...
//                         {14, 16}, {11, 12}};
  const int edge[][2] = {{15, 13}, {13, 11}, {16, 14}, {14, 12}, {11, 12}, {5, 11}, {6, 12}, {5, 6}, {5, 7}, {6, 8}, {7, 9}, {8, 10}, {1, 2}, {0, 1}, {0, 2}, {1, 3}, {2, 4}, {3, 5}, {4, 6}};

  const int color[][3] = {{121,9,254}, {102,38,254}, {70,87,250}, {43,126,246}, {4,185,234}, {38,217,222}, {58,232,214},
                          {79,245,204}, {102,254,187}, {126,254,168}, {153,254,149}, {181,254,131}, {210,254,114}, {240,254,98},
                          {254,240,79}, {254,210,58}, {254,181,38}, {254,153,19}, {254,126,0}, {254,102,0}, {254,79,0}, {254,58,0},
                          {254,38,0}, {254,19,0}, {254,0,0}, {240,0,0}, {210,0,0}, {181,0,0}, {153,0,0}, {126,0,0}, {102,0,0},
                          {79,0,0}, {58,0,0}, {38,0,0}, {19,0,0}, {0,0,0}};
  ...
  for (int batchid = 0; batchid < results_kpts.size(); batchid++) {
    for (int i = 0; i < results_kpts[batchid].num_joints; i++) {
      if (results_kpts[batchid].keypoints[i * 3] > kpts_threshold) {
        ...
        cv::circle(*rgbaImage, cv::Point2d(x_coord, y_coord), 5,
    ...
    for (int i = 0; i < results_kpts[batchid].num_joints; i++) {
      ...
      cv::line(*rgbaImage, cv::Point2d(x_start, y_start),
               cv::Point2d(x_end, y_end), cv::Scalar(color[i][0], color[i][1], color[i][2]), 5, 16);
    }
  }
}
...
void Pipeline::Action_Process(cv::Mat *rgbaImage,
  ...
  if (single_person) {
    ...
    if (!results.empty()) {
      // TODO:
      action_count = single_action_check(agcn_, results_kpts[0].keypoints, results[0].h*rgbaImage->rows, actionid, 0);
    }
  }
  //可以有但沒必要
  else {
...
std::vector<int> Pipeline::GetCountAndScore() {

  return std::vector<int> {get_action_count(0),get_action_score(0)};
}
